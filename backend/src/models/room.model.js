...
const roomSchema = new mongoose.Schema(
  {
    room_name: {
      ...
      trim: true,
      minlength: [ROOM_NAME_MIN_LENGTH, `Le nom de la salle doit contenir au moins ${ROOM_NAME_MIN_LENGTH} caractères`],
      maxlength: [ROOM_NAME_MAX_LENGTH, `Le nom de la salle ne peut pas dépasser ${ROOM_NAME_MAX_LENGTH} caractères`],
      match: [/^[\w\s-éèêëàâäôöùûüç]+$/i, "Le nom de la salle contient des caractères non autorisés"]
    },
    description: {
      type: String,
      trim: true,
      maxlength: [DESCRIPTION_MAX_LENGTH, `La description ne peut pas dépasser ${DESCRIPTION_MAX_LENGTH} caractères`]
    },
    members: [
      {
