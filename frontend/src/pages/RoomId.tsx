...
const RoomId = () => {
  ...
  useEffect(() => {
    const loadRooms = async () => {
      if (room.length === 0) {
        // Ne charge que si pas encore chargé
        try {
          await getRoom();
        } catch (err) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "done":
        return <CheckmarkIcon className="w-4 h-4" />;
      default:
              ...
              <div className="space-y-3">
                {currentRoom.members?.length ? (
                  currentRoom.members.map((member) => (
                    <div
                      key={member._id}
                      className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-800/30 transition-colors duration-200"
                    >
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
              ...
              <Link
                ...
                className="px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 flex items-center gap-2 shadow-lg hover:shadow-xl"
              >
            ...
            {currentRoom.tasks && currentRoom.tasks.length > 0 ? (
              ...
              <div className="bg-slate-300 backdrop-blur-sm rounded-2xl p-12 text-center shadow-md">
                ...
                <Link
                  to={"/dashboard/add/task"}
                  className="px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 flex items-center gap-2 mx-auto shadow-lg"
                >
